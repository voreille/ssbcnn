...
def bispectrum_conv3d(X, out_ch, ksize, strides=(1, 1, 1, 1, 1), padding='VALID', degreeMax=1, stddev=0.4,
...
def get_bispectrum_invariant_coeffs(R, degreeMax, cg_mat):
    '''
    Returns the bispectrum invariants from the responses to the SHs
...
#        for n2 in range(n1, math.ceil(degreeMax / 2) + 1):
    ...
    ## TO TEST with the jupyter notebook
    # return tf.reshape(b,[bs,rsh[1],rsh[2],rsh[3],b.get_shape().as_list()[-1]])
